...
namespace BackboneLogger
{
    ...
    public sealed class LoggerConfig : ScriptableObject
    {
        #region STATIC
        ...
        public static LoggerConfig Instance
        {
            get
            {
                ...
                {
#if UNITY_EDITOR
                    // Find AssetDatabase when editor context
                    string[] guids = UnityEditor.AssetDatabase.FindAssets($"t:{nameof(LoggerConfig)}");
                    if (guids.Length == 0)
                    {
                        Debug.LogError($"No assets found in '{nameof(LoggerConfig)}'!");
                        return null;
                    }
                    ...
                    if (guids.Length > 0)
                    {
                        ...
                        instance = UnityEditor.AssetDatabase.LoadAssetAtPath<LoggerConfig>(path);
                    }
#else
        ...
        private void Awake()
        {
            ...
            {
                // Show warning...
                Debug.LogWarning($"Duplicate {nameof(LoggerConfig)} detected: {name}");
            }
        }
        ...
        /// <summary>
        /// Controls if categories must be display grouped by prefix.
        /// </summary>
        ...
        public void EnsureDefaultCategories()
        {
            ...
            {
                if (categories.All(c => c.name != def.name))
                {
