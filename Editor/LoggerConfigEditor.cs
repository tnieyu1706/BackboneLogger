...
namespace BackboneLogger
{
    ...
    {
        private readonly NamedBuildTarget namedBuildTarget = NamedBuildTarget.Standalone;
        ...
        private LoggerConfig importSource;
        ...
        {
            // Starting read/write of serialized data
            ...
            bool isEnabled = IsSymbolDefined(SYMBOL, namedBuildTarget);
            bool newValue = EditorGUILayout.Toggle("Enable Logging", isEnabled);

            if (EditorGUI.EndChangeCheck())
            {
                if (newValue) AddDefineSymbol(SYMBOL, namedBuildTarget);
                else RemoveDefineSymbol(SYMBOL, namedBuildTarget);
        ...
        private void DrawCategoriesGroupedByPrefix()
        {
            ...
            foreach (var key in keys)
            {
                ...
                if (GUILayout.Button("Delete Group", GUILayout.Width(100)))
                {
                            ...
                            "Delete", "Cancel"))
                    {
                        ...
                        for (int n = indexList.Count - 1; n >= 0; n--)
                            categoriesProp.DeleteArrayElementAtIndex(indexList[n]);
                ...
                // When user toggled the group, set all children
        ...
        /// \param index (I) of the category in the categories array (used for deletion).
        /// \details
        /// Row layout:
